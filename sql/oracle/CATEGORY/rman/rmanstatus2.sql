...
--DESCRIBE: detailed output of RMAN backup history
...
--   j.status,
decode(j.status, 'COMPLETED WITH ERRORS', 'COMP_WITH_ERR', 'COMPLETED WITH WARNINGS', 'COMP_WITH_WARN', j.status) status,
j.input_type,
...
select rs.recid,
       to_char(rs.start_time, 'yyyy-mm-dd hh24:mi:ss') start_time,
       to_char(rs.end_time, 'yyyy-mm-dd hh24:mi:ss') end_time,
       rbjd.time_taken_display,
       rs.operation,
       decode(rs.status, 'COMPLETED WITH ERRORS', 'COMP_WITH_ERR', 'COMPLETED WITH WARNINGS', 'COMP_WITH_WARN', rs.status) status,
       to_char(rs.start_time, 'DAY') START_DOW,
       rs.mbytes_processed,
       rs.object_type,
...
where rs.start_time > trunc(sysdate)-&days
  and rs.session_recid = rbjd.session_recid (+)
order by start_time;
